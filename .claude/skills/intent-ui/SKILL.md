...
- `back?: { href, label } | false` — override the header back link, or pass `false` to hide it. Allowed
  hrefs are the same as everywhere in a flow: `#/` or `#/intents/<other-slug>`, never a platform record list.
...
<EntitySelectStep
  ...
  createDialog={showCreate && (
    <div className="rounded-2xl border p-4 space-y-3">
      {/* only the fields needed for a quick registration — the rest is filled later on the platform */}
...
- Step "Pick a Kunde": search + list, plus an **inline mini-form** with only the
  ...
  No modal, no photo-scan UI, no address fields — those can be filled later on
  the platform if ever needed.
...
- ❌ **Filtered table** of one entity → that's the platform record list
- ❌ **Single-entity form** with styling → that's the existing dialog
- ❌ **Read-only summary/stats** → belongs on the dashboard
- ❌ **Entity list with action buttons** → that's the platform record list with extra buttons

---

## CRITICAL: Never link the user from an intent UI to a platform record list

The platform's record lists (the sidebar already links there) are generic admin
...
The sidebar is responsible for navigation to a platform record list if the user needs it.
...
- **No `{Entity}Dialog`**: create records with a step-tailored mini-form + `LivingAppsService.create…Entry()` — see the CRITICAL section above.
...
skip a required address (it gets filled later on the platform), a booking may not skip its date. When you
...
the whole step, and `tsc` never sees it. Use the sentinel the scaffold uses
