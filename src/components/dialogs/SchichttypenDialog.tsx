/**
 * SchichttypenDialog — pre-generated create/edit dialog for Schichttypen.
 *
 ...
 * SchichttypenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Schichttypen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<SchichttypenDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Schichttypen';
...
/** Widened prefill type for SchichttypenDialog.defaultValues — see file header. */
export type SchichttypenDialogDefaults = Schichttypen['fields'];

interface SchichttypenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Schichttypen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: SchichttypenDialogDefaults;
...
export function SchichttypenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: SchichttypenDialogProps) {
  const [fields, setFields] = useState<Partial<Schichttypen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['bezeichnung', 'startzeit', 'endzeit'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Schichttypen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'schichttypen');
      await onSubmit(clean as Schichttypen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Schichttypen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('schichttypen') })
    : t('new_entity', { entity: appLabel('schichttypen') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        <Label htmlFor="bezeichnung">{fieldLabel('schichttypen', 'bezeichnung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="bezeichnung"
    ...
    'kuerzel': (
      <div key="kuerzel" className="space-y-1.5">
        <Label htmlFor="kuerzel">{fieldLabel('schichttypen', 'kuerzel')}</Label>
        <Input
          id="kuerzel"
          placeholder="z. B. FS"
    ...
    'startzeit': (
      <div key="startzeit" className="space-y-1.5">
        <Label htmlFor="startzeit">{fieldLabel('schichttypen', 'startzeit')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="startzeit"
    ...
    'endzeit': (
      <div key="endzeit" className="space-y-1.5">
        <Label htmlFor="endzeit">{fieldLabel('schichttypen', 'endzeit')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="endzeit"
          ...
          required
        />
        {showErrors && !fields.endzeit && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'pausendauer': (
      <div key="pausendauer" className="space-y-1.5">
        <Label htmlFor="pausendauer">{fieldLabel('schichttypen', 'pausendauer')}</Label>
        <Input
          id="pausendauer"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'pausendauer')}
          placeholder="z. B. 30"
          value={fields.pausendauer !== undefined ? fields.pausendauer : (computedValues['pausendauer'] ?? '')}
          onChange={e => setFields(f => ({ ...f, pausendauer: clampNumberValue(formEnhancements, 'pausendauer', e.target.value) }))}
        />
      </div>
    ),
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        <Label htmlFor="beschreibung">{fieldLabel('schichttypen', 'beschreibung')}</Label>
        <Textarea
          id="beschreibung"
          placeholder="Arbeitszeiten, besondere Anforderungen, Besetzung..."
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Bezeichnung", "kuerzel": "Kürzel", "startzeit": "Startzeit", "endzeit": "Endzeit", "pausendauer": "Pausendauer (Minuten)", "beschreibung": "Beschreibung"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SCHICHTTYPEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
