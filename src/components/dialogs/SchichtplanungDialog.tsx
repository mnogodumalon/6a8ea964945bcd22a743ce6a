/**
 * SchichtplanungDialog — pre-generated create/edit dialog for Schichtplanung.
 *
 * Props: open, onClose, onSubmit(fields) => Promise<void>, defaultValues?,
 * recordId? (pass when EDITING — enables the attachments section),
 * mitarbeiterverwaltungList (full hook array — resolves the Mitarbeiterverwaltung applookup),
 * schichttypenList (full hook array — resolves the Schichttypen applookup),
 * enablePhotoScan?, enablePhotoLocation?.
 *
 * defaultValues is SHAPE-TOLERANT and its prop type is the EXPORTED
 * SchichtplanungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Schichtplanung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<SchichtplanungDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Schichtplanung';
...
/** Widened prefill type for SchichtplanungDialog.defaultValues — see file header. */
export type SchichtplanungDialogDefaults = Schichtplanung['fields'];

interface SchichtplanungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Schichtplanung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: SchichtplanungDialogDefaults;
...
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  ...
  schichttyp: APP_IDS.SCHICHTTYPEN,
};
...
export function SchichtplanungDialog({ open, onClose, onSubmit, defaultValues, recordId, mitarbeiterverwaltungList, schichttypenList, enablePhotoScan = true, enablePhotoLocation = true }: SchichtplanungDialogProps) {
  const [fields, setFields] = useState<Partial<Schichtplanung['fields']>>({});
  ...
  // Inline-Create state for "Schichttypen" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraSchichttypen` list, and select it in
  // the originating Combobox via the captured `createSchichttypenField`.
  const [createSchichttypenOpen, setCreateSchichttypenOpen] = useState(false);
  const [createSchichttypenInitial, setCreateSchichttypenInitial] = useState('');
  const [createSchichttypenField, setCreateSchichttypenField] = useState<string>('');
  const [extraSchichttypen, setExtraSchichttypen] = useState< Schichttypen[]>([]);
  const schichttypenListAll = useMemo(
    () => [...schichttypenList, ...extraSchichttypen],
    [schichttypenList, extraSchichttypen],
  );
  function openCreateSchichttypen(fieldKey: string, q: string) {
    setCreateSchichttypenField(fieldKey);
    setCreateSchichttypenInitial(q);
    setCreateSchichttypenOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['datum', 'mitarbeiter', 'schichttyp'] as const;
  ...
  const computedContext = useMemo<ComputedContext>(() => ({
    lookupLists: {
      ...
      'schichttyp': schichttypenList,
    },
  }), [mitarbeiterverwaltungList, schichttypenList, ]);
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Schichtplanung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'schichtplanung');
      await onSubmit(clean as Schichtplanung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="schichttyp" entity="Schichttypen">\n${JSON.stringify(schichttypenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitarbeiter", "schichttyp"]);
        ...
        const schichttypName = raw['schichttyp'] as string | null;
        if (schichttypName) {
          const schichttypMatch = schichttypenList.find(r => matchName(schichttypName!, [String(r.fields.bezeichnung ?? '')]));
          if (schichttypMatch) merged['schichttyp'] = createRecordUrl(APP_IDS.SCHICHTTYPEN, schichttypMatch.record_id);
        }
        return merged as Partial<Schichtplanung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('schichtplanung') })
    : t('new_entity', { entity: appLabel('schichtplanung') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'datum': (
      <div key="datum" className="space-y-1.5">
        <Label htmlFor="datum">{fieldLabel('schichtplanung', 'datum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="datum"
    ...
    'mitarbeiter': (
      <div key="mitarbeiter" className="space-y-1.5">
        <Label htmlFor="mitarbeiter">{fieldLabel('schichtplanung', 'mitarbeiter')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="mitarbeiter"
          ...
          createLabel={t('create_in', { entity: appLabel('mitarbeiterverwaltung') })}
        />
    ...
    'schichttyp': (
      <div key="schichttyp" className="space-y-1.5">
        <Label htmlFor="schichttyp">{fieldLabel('schichtplanung', 'schichttyp')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="schichttyp"
          placeholder="Schichttyp wählen"
          items={schichttypenListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.bezeichnung ?? r.record_id),
          }))}
          value={extractRecordId(fields.schichttyp)}
          onChange={id => setFields(f => ({ ...f, schichttyp: id ? createRecordUrl(APP_IDS.SCHICHTTYPEN, id) : undefined }))}
          onCreateNew={(q) => openCreateSchichttypen("schichttyp", q)}
          createLabel={t('create_in', { entity: appLabel('schichttypen') })}
        />
        {showErrors && !fields.schichttyp && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'bereich': (
      <div key="bereich" className="space-y-1.5">
        <Label htmlFor="bereich">{fieldLabel('schichtplanung', 'bereich')}</Label>
        <Input
          id="bereich"
          placeholder="z. B. Kasse, Lager"
          value={fields.bereich ?? ''}
          onChange={e => setFields(f => ({ ...f, bereich: e.target.value }))}
        />
      </div>
    ),
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        <Label htmlFor="anmerkungen">{fieldLabel('schichtplanung', 'anmerkungen')}</Label>
        <Textarea
          id="anmerkungen"
          placeholder="Spezielle Anweisungen, Vertretungen, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"datum": "Datum", "mitarbeiter": "Mitarbeiter", "schichttyp": "Schichttyp", "bereich": "Bereich / Standort", "anmerkungen": "Anmerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitarbeiter": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "abteilung": "Abteilung", "position": "Position / Rolle", "beschaeftigungsart": "Beschäftigungsart", "notizen": "Notizen"}, "schichttyp": {"bezeichnung": "Bezeichnung", "kuerzel": "Kürzel", "startzeit": "Startzeit", "endzeit": "Endzeit", "pausendauer": "Pausendauer (Minuten)", "beschreibung": "Beschreibung"}};
  ...
  return (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SCHICHTPLANUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createSchichttypenOpen && (
      <SchichttypenDialog
        open={createSchichttypenOpen}
        onClose={() => setCreateSchichttypenOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createSchichttypenEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Schichttypen;
            setExtraSchichttypen(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.SCHICHTTYPEN, result.id);
            setFields(prev => ({ ...prev, [createSchichttypenField]: url } as any));
          }
          setCreateSchichttypenOpen(false);
        }}
        defaultValues={createSchichttypenInitial
          ? ({ bezeichnung: createSchichttypenInitial } as any)
          : undefined}
      />
    )}
    </>
